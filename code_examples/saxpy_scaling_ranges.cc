...
// assume a container
template <typename T>
void saxpy(std::span<T> z, const typename T::value_type A, const std::span<T> x,
    const std::span<T> y)
{
  auto single_saxpy = [A](auto x, auto y) { return A * x + y; };

  z = std::views::zip_transform(single_saxpy, x, y);
}
